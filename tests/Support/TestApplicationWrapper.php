...
use Partoo\Widgets\NamespacesRepository;
...
class TestApplicationWrapper implements ApplicationWrapperContract
{
     ...
     */
    public $config = [
        'laravel-widgets.default_namespace' => 'Partoo\Widgets\Test\Dummies',
    ...
    {
        if ($abstract == 'Partoo.widget') {
            return new WidgetFactory($this);
        }

        if ($abstract == 'Partoo.async-widget') {
    ...
    {
        if ($id == 'Partoo.widget-namespaces') {
            return (new NamespacesRepository())->registerNamespace('dummy', '\Partoo\Widgets\Test\Dummies');
        }
    }
}
