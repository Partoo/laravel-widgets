...
use Partoo\Widgets\Test\Dummies\TestCachedWidget;
...
class WidgetFactoryTest extends TestCase
{
    ...
    {
        $this->expectException('\Partoo\Widgets\Misc\InvalidWidgetClassException');
    ...
    {
        $output = $this->factory->run('\Partoo\Widgets\Test\Dummies\TestDefaultSlider');
    ...
    {
        $this->expectException('Partoo\Widgets\Misc\NamespaceNotFoundException');
    ...
    public function testItCanRunReloadableWidgets()
    {
        ...
        $this->assertEquals(
            '<div id="Partoo-widget-container-1" style="display:inline" class="Partoo-widget-container">Feed was executed with $slides = 6' .
            '<script type="text/javascript">' .
            'setTimeout( function() {' .
            'var widgetTimer1 = setInterval(function() {' .
            'if (window.$) {' .
            "$('#Partoo-widget-container-1').load('" . $this->ajaxUrl('TestRepeatableFeed') . "');" .
    ...
    public function testWidgetContainerCanBeCustomized()
    {
        ...
        $this->assertEquals(
            '<p id="Partoo-widget-container-1" data-id="123">Dummy Content' .
            '<script type="text/javascript">' .
            'setTimeout( function() {' .
            'var widgetTimer1 = setInterval(function() {' .
            'if (window.$) {' .
            "$('#Partoo-widget-container-1').load('" . $this->ajaxUrl('TestWidgetWithCustomContainer') . "');" .
            'clearInterval(widgetTimer1);' .
            '}' .
            '}, 100);' .
            '}, 10000)' .
    ...
    public function testItCanCacheWidgets()
    {
        ...
        $key = 'Partoo.widgets.' . serialize(['testCachedWidget', ['foo' => 'bar']]);
