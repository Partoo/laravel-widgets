...
namespace Partoo\Widgets\Console;
...
class WidgetMakeCommand extends GeneratorCommand
{
    ...
    {
        $this->desc = $this->ask('The widget description (optional)');
    ...
    {
        if ($this->files->exists($path = base_path('resources/views') . '/widgets/manifest.json')) {
            $data = json_decode($this->files->get($path), true);
            $new_data = ['name' => $this->makeViewName(), 'desc' => $this->desc];
            $data = json_encode([$data, $new_data], JSON_UNESCAPED_UNICODE);
            $this->files->put($path, $data);
        } else {
            $data = ['name' => $this->makeViewName(), 'desc' => $this->desc];
            $this->files->put($path, json_encode($data, JSON_UNESCAPED_UNICODE));
        }
    }
