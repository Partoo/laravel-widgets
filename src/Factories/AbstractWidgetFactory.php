...
namespace Partoo\Widgets\Factories;

use Illuminate\Support\Str;
use Partoo\Widgets\AbstractWidget;
use Partoo\Widgets\Contracts\ApplicationWrapperContract;
use Partoo\Widgets\Misc\EncryptException;
use Partoo\Widgets\Misc\InvalidWidgetClassException;
use Partoo\Widgets\Misc\ViewExpressionTrait;
...
abstract class AbstractWidgetFactory
{
    ...
    protected function instantiateWidget(array $params = [])
    {
        ...
        if (preg_match('#^(.*?)::(.*?)$#', $str, $m)) {
            $rootNamespace = $this->app->get('Partoo.widget-namespaces')->getNamespace($m[1]);
            $str = $m[2];
        }
        ...
        if (!is_subclass_of($widgetClass, 'Partoo\Widgets\AbstractWidget')) {
            throw new InvalidWidgetClassException('Class "' . $widgetClass . '" must extend "Partoo\Widgets\AbstractWidget" class');
        }
